...
object RookMoves {
    ...
    private fun computeRowColumnMasks(): Array<ULong> {
        val masks = Array(64){0uL}
        for (square in 0 until 64) {
            masks[square] = computeRowColumnMask(square)
        }
        return masks
    }

    private fun computeRowColumnMask(square: Int): ULong {
        val file = square % 8
        val rank = square / 8
        var mask = 0uL

        for (r in 0 until 8) {
            if (r != rank) mask = mask or (1uL shl (r * 8 + file))
        }

        for (f in 0 until 8) {
            if (f != file) mask = mask or (1uL shl (rank * 8 + f))
        }
